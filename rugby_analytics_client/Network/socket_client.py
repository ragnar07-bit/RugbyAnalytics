...
class SocketClient:
    ...
    def connect(self, on_packet_received_callback, on_disconnect_callback):
        try:
            ...
            print(f"[NETWORK] Connesso con successo al server {self.host}:{self.port}")
            ...
            print(f"[NETWORK ERROR] Impossibile stabilire la connesione: {e}")
            self.is_connected = False
    ...
    def send_packet(self, packet: Packet) -> bool:
        if not self.is_connected or not self.client_socket:
            print("[NETWORK ERROR] Invio fallito: Socket non connesso!!")
        ...
        try:
            ...
            print(f"[NETWORK SEND] Inviato pacchetto con Action: '{packet.action}'")
            
            return True
        except Exception as e:
            print(f"[NETWORK ERROR] Errore durante la trasmissione: {e}")
    ...
    def _listen_to_server(self, on_packet_received_callback, on_disconnect_callback):
        ...
        while self.is_connected:
            try:
                ...
                print(f"[NETWORK ERROR] Connessione chiussa inaspettatamente: {e}")
                break
    ...
    def disconnect(self):
        ...
        if self.client_socket:
            self.client_socket.close()
        print("[NETWORK] Socket chiuso dall'applicazione client!")